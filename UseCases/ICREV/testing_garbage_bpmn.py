...
#Tokencreation
Token_oda6dskBbFwdjo5h8VE_3w = None
#Here we try to evaluate the Expression `counter` = None with diffrent engines
for engine in [{}, {'engine': 'python'}]:
    try:
        Token_oda6dskBbFwdjo5h8VE_3w = Token_oda6dskBbFwdjo5h8VE_3w.eval('`counter` = None', **engine)
    except Exception:
...
for engine in [{}, {'engine': 'python'}]:
    try:
        Token_oda6dskBbFwdjo5h8VE_3w = Token_oda6dskBbFwdjo5h8VE_3w.eval('`other` = None', **engine)
    except Exception:
        print(engine, "failed to evaluate", '`other` = None', "trying next")

#add row based on args order
Token_oda6dskBbFwdjo5h8VE_3w.loc[len(Token_oda6dskBbFwdjo5h8VE_3w.index)] = [1, 'test']
#Here we try to evaluate the Expression `counter` = `counter`+1 with diffrent engines
for engine in [{}, {'engine': 'python'}]:
    try:
        Token_oda6dskBbFwdjo5h8VE_3w = Token_oda6dskBbFwdjo5h8VE_3w.eval('`counter` = `counter`+1', **engine)
    except Exception:
...
#Creating new Dataframe based on Token_oda6dskBbFwdjo5h8VE_3w
Token_8R8aYld6vPT4FcEoVc0skw = Token_oda6dskBbFwdjo5h8VE_3w.copy(True)

#Here we try to query the Expression `counter` == 10 with diffrent engines
for engine in [{'engine': 'numexpr'}, {'engine': 'python'}]:
    try:
        Token_8R8aYld6vPT4FcEoVc0skw = Token_8R8aYld6vPT4FcEoVc0skw.query('`counter` == 10', **engine)
    except Exception:
...
for engine in [{}, {'engine': 'python'}]:
    try:
        Token_8R8aYld6vPT4FcEoVc0skw = Token_8R8aYld6vPT4FcEoVc0skw.eval('`other` = @replace_all(`other`, "t","T")', **engine)
    except Exception:
        print(engine, "failed to evaluate", '`other` = @replace_all(`other`, "t","T")', "trying next")


#here we save the data from Token_8R8aYld6vPT4FcEoVc0skw to the the counter.xlsx into the  sheet Sheet1 and dont use the index
Token_8R8aYld6vPT4FcEoVc0skw.to_excel("counter.xlsx", sheet_name = "Sheet1", index = False)
